...
contract ProxyController_v1 is VersionContract {
    ...
    function transferWithSign(bytes _sign, address _tokenAddress, address _from, address _to, uint _amount, uint _nonce, bytes _clientSign) public {
        // TODO
	logic_v1.transferWithSign(_tokenAddress, _from, _to, _amount, _nonce, _clientSign);
    }
    
    function CreatePool(bytes _sign, address _tokenAddress, address _pool, uint _nonce, bytes _clientSign) public {
        logic_v1.CreatePool(_tokenAddress, _pool, _nonce, _clientSign);	
    }
    
    function AddMember(bytes _sign, address _tokenAddress, address _pool, address _new, uint _nonce, bytes _clientSign) public {
        logic_v1.AddMember(_tokenAddress, _pool, _new, _nonce, _clientSign);
    }
}
